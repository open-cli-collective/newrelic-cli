...
// TestValidateFormat_ClosedSet pins the cli-common
// docs/output-and-rendering.md §2 closed-set policy: only `table` and `plain`
// are valid global -o values. `json` is intentionally rejected so any future
// resource-command JSON branch is unreachable; control-plane envelopes
// (set-credential / config show / config test --json) and passthrough
// surfaces (nerdgraph / nrql) emit JSON via their own local encoders.
func TestValidateFormat_ClosedSet(t *testing.T) {
	tests := []struct {
		name    string
		format  string
		wantErr bool
		...
		{"rejects json (closed-set carve-out is per-subcommand --json)", "json", true},
		{"rejects yaml", "yaml", true},
		{"rejects xml", "xml", true},
		{"rejects empty", "", true},
	}
...
func TestView_Render_Table(t *testing.T) {
	...
	err := v.Render([]string{"ID", "NAME"}, [][]string{{"1", "Test"}})
	require.NoError(t, err)
